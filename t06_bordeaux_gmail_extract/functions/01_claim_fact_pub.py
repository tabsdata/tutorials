...
class GmailPublisher(td.SourcePlugin):
    ...
    def chunk(self, working_dir: str) -> str:
        ...
        email_user = os.getenv("imap_email_user")
        email_password = os.getenv("imap_email_password")

        mail = imaplib.IMAP4_SSL(imap_host, imap_port)
        mail.login(email_user, email_password)
        mail.select("inbox")
        date_since = (datetime.now() - timedelta(days=200)).strftime("%d-%b-%Y")
                ...
                for part in msg.walk():
                    if part.get_content_disposition() == "attachment":
                        ...
                        if (
                            ...
                            and filename.startswith("TPA_Claim_Data")
                        ):
