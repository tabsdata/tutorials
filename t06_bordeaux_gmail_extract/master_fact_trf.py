...
def master_fact_trf(policy_dim: td.TableFrame, claims_fact: td.TableFrame):
    ...
    master = master.with_columns([pl.coalesce(col + '_right', col).alias(col) for col in overlapping])
    master = master.select([col for col in master.columns() if col not in [i + '_right' for i in overlapping]])
